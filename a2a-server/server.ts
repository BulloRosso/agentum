...
import { TaskHandler, TaskContext } from "./handler.js";
...
export class A2AServer {
   ...
   * @param port Port number to listen on. Defaults to 3200.
   * @returns The running Express application instance.
   */
  start(port = 3200): express.Express {
    const app = express();
    ...
    app.listen(port, () => {
      console.log(
        `A2A Server listening on port ${port}`
      );
    });
  ...
  private errorHandler = (
    ...
    if (err instanceof A2AError) {
      ...
      try {
        ...
        const body = req.body ? JSON.parse(JSON.stringify(req.body)) : null;
        reqId = body?.id ?? null;
      } catch (_) {
