// Import necessary types from A2ASchema.ts
import {
  // Core types
  AgentCard,
  JSONRPCRequest,
...
export class A2AClient {
  ...
  async supports(capability: "streaming" | "pushNotifications"): Promise<boolean> {
    try {
      ...
      switch (capability) {
        // Check for string capabilities in the capabilities array
        case "streaming":
          return card.capabilities?.includes("streaming") || false;
        case "pushNotifications":
          return card.capabilities?.includes("pushNotifications") || false;
        default:
          return false;
      }
    } catch (error) {
